...
**The drum machine after it — 16 steps, 8 tracks, a sampler, an editable grid — closed on 2026-08-21**, and what closed it was live running rather than code: no clicks by ear on tempo, grid and fader across all three browsers, thirty minutes with the kit reloaded and the arena not growing, and 4.5–4.9% of a core in a hidden tab with all 128 cells struck, against the 5% the criterion asks for. Two things are worth carrying out of those runs. The CPU figure is a hidden-tab figure by the criterion's own instruction — the foreground costs 14%, and that is the page painting, not the engine; the arithmetic that predicted 8.5% was taken on a page with no grid and no playhead canvas and does not describe this one. And **a run nobody wrote down does not exist**: all three of those had been done before the document said so, and the document said "not run" the whole time. `Op` and `Command` stay synchronised by hand — generating them would still leave `Command` and its payloads to write out, so the manual list is the honest one — but the gap that made it risky at eight opcodes is closed: `tests::OPCODES` is the table both directions are checked against. What remains open from the skeleton is that a clamped BPM is corrected in silence, with no channel to say so.
...
**A third clock runs on the page, and it is the only one that has to survive a hidden tab.** `Diagnostics.svelte` folds away the evidence each milestone was checked against, and one of its rows is an instrument rather than a reading: the worklet publishes a rendered-frame counter into the SAB, the page samples it once a second, and `renderDrift` turns two samples `DRIFT_WINDOW` intervals apart into milliseconds of render time lost per second. Two things about it are worth carrying before touching it. **`requestAnimationFrame` is disqualified** — a sampler on the frame clock sleeps exactly when the tab is backgrounded, which is the one condition anybody wanted measured — and that is why `IntervalLoop` is a type of its own despite being shaped exactly like `FrameLoop`. And **this is not the underrun detector the skeleton is missing**: what it measures is sustained overload of the render thread, a click's worth of trouble was never in its reach, and a zero here says nothing at all about dropouts. Why the window is eight intervals wide, and why the sample count and the seconds it spans are printed beside the number, are at the constants in `session.svelte.ts` — both measured rather than chosen.

**`ui/` divides by what a test can reach, exactly as `worklet/` does.** Nothing in this package can execute a `.svelte` file: the suite runs under Node with no DOM and no component runner. So arithmetic lives in plain modules with specs beside them — `ui/paint.ts` for where the playhead and the meters go, `ui/format.ts` for the transport clock — and the components keep the element, its size and the subscription. The line is not tidiness: of the four defects the UI step produced, **none was found by reading** — three by looking at the running page and one by moving arithmetic out of a component into a module with a spec beside it. The dressing pass after it added a second kind of defect that reading cannot reach, and it is not layout: a mark can sit exactly where it was computed and still not exist on the page — the beat rule was drawn in the border token, a neighbour of the 128 cell borders it has to be told apart from. Geometry and weight are separate questions, and forcing an unmissable colour is what separates them before anything gets "fixed".
...
**A fourth thing is under the version and has no mirror at all: the SAB layout in `audio/ring.ts`.** One file, read by both threads, so nothing inside one source tree can disagree with it — which is exactly what makes it easy to leave outside the version's scope. But two artifacts are built from that one file, and a worklet bundle that was not rebuilt is a second layout: page and processor addressing different words of the same buffer, with the record format and the telemetry block both still agreeing. So moving any word in the SAB bumps `PROTOCOL_VERSION` alongside the two ABI shapes. **The one exception is a word appended past the end** — no offset moves, and a stale bundle gives itself away without the version's help, by a counter that never grows. `WORD_RENDERED_FRAMES` arrived that way.

The guards:

- `PROTOCOL_VERSION` — **its scope is all four**, and that is what makes it useful: anything left outside its scope is where neither check below fires and the symptom is wrong values rather than silence. Bump on any change to any of them.
...
- `worklet/exchange.ts` — the quantum's two copies: command bytes SAB → linear memory before the engine runs, the telemetry block back the other way under the seqlock after it. Apart from `render.ts` because what it answers to is the ring's orderings rather than the buffer's, and out of `processor.ts` because both copies are reachable from a test and the class around them is not.
...
- `audio/ring-writer.ts` — the page's end of the command ring, and the only place a record is ever written. Kept out of `ring.ts` because that file is a layout both threads read and this is a road only one of them has.
- `audio/worklet-messages.ts` — what the two threads say to each other outside the command protocol: what the audio thread learns about its environment and can report no other way, and the one thing travelling forward, a kit. Imported by both sides on purpose — the worklet is bundled standalone, so a shared type is the only thing holding the two ends together.
...
- `ui/*.svelte` — the same rule as `processor.ts`, one level up: nothing here can run a `.svelte` file, so what stays in one is the markup, the styles that are its own, the element and the subscription.
...
A comment argues for itself, in front of whoever is reading the repository. Planning for this project happens in a document that is not in git, and **a reference to it does not belong in code**: a pointer to a file the reader cannot open explains nothing, and the reasoning it stands in for is exactly what the comment was supposed to carry. Write the argument out. This has been cleaned out of the code three times already — the second time from comments written a day after the first, the third from four references in two spec files, written a week and then twelve days after that. **So grep for it rather than trusting the rule**: `grep -rn '§' crates web/src web/tests scripts` has to stay silent, and it is worth running after writing a comment that wanted to cite something.
